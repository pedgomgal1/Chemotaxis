function [avgSpeedRoundT, stdSpeedRoundT, semSpeedRoundT,avgMeanSpeed,avgStdSpeed,avgSemSpeed,avgAccelerationRoundT, stdAccelerationRoundT, semAccelerationRoundT,avgMeanAcceleration,avgStdAcceleration,avgSemAcceleration,speedFileRoundT,accelerationFileRoundT] = calculateAverageSpeedAndAcceleration(speedFile)
    ...
    for nLab = 1:length(uniqueLabels)
        ...
        accelerationFileRoundT = [accelerationFileRoundT; [ones(sum(idsLab),1)*uniqueLabels(nLab),auxUniqRoundTime, dV]];
    ...
    avgAccelerationRoundT=arrayfun(@(x) mean(accelerationFileRoundT(ismember(accelerationFileRoundT(:,2),x),3)),uniqRoundTime);
    semAccelerationRoundT=arrayfun(@(x) std(accelerationFileRoundT(ismember(accelerationFileRoundT(:,2),x),3))/sqrt(sum(ismember(accelerationFileRoundT(:,2),x))),uniqRoundTime);
    stdAccelerationRoundT=arrayfun(@(x) std(accelerationFileRoundT(ismember(accelerationFileRoundT(:,2),x),3)),uniqRoundTime);
